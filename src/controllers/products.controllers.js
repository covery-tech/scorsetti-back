      ...
      if (err) res.status(400).send(err);
        ...
        if (result.length) {
          let data = [
            ...
            result[0].acc_personal === "habilitado" && "acc_personal",
...
const getAllOrders = (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).send("Error al obtener los datos de las tablas 'orders' y 'orders_records'");
  }
};

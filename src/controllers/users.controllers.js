...
class UserController {
    ...
    const user = await UserModels.register(name,lastName,date,password,email)
    res.status(200).send(user)
    }
    static async postLogin (req,res) {
        const {email, password } = req.body
        if(!email || !password || !password.length || !email.length) return res.status(400).send("missing data")
